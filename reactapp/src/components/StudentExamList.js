...
export default function StudentExamList() {
    ...
    // Fetch completed exams/results for the student
    const fetchResults = async () => {
        try {
            setLoadingResults(true);
            const res = await api.getStudentResults(studentUsername);
            setResults(res.data || []);
        } catch (e) {
            // Don't block the page if results fail
            setResults([]);
        } finally {
    ...
    // View result for a completed exam
    const handleViewResult = (studentExamId) => {
            ...
            <div className="container">
                ...
                <div className="list-container" style={{ marginTop: '2rem' }}>
                    <div className="list-header">
                        ...
                        <span className="list-count">{results.length} completed</span>
                    </div>
                    <div className="list-body">
                        {loadingResults ? (
                            ...
                            results.map((result) => (
                                <div key={result.studentExamId} className="list-item">
                                    ...
                                    <div className="item-stats">
                                        ...
                                        <div className="stat-item">
                                            <span className="stat-icon">📅</span>
                                            <span>Date: {formatDate(result.completedAt)}</span>
                                        </div>
                                    </div>
                                    <div className="item-actions">
                                        <button
                                            className="action-btn-small btn-view"
                                            onClick={() => handleViewResult(result.studentExamId)}
                                        >
                                            View Result
                                        </button>
                                    </div>
